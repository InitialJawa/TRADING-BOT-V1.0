...
STRATEGY_MAP = {
    "a": {"name": "A D1 Long Bias",   "tf": "D1", "tf_mt5": 16408,  "bars": 1000,  "dd_limit": 50},
    "b": {"name": "B H4 EMA Cross",   "tf": "H4", "tf_mt5": 16388,  "bars": 2000,  "dd_limit": 50},
    "c": {"name": "C H4 PSAR",        "tf": "H4", "tf_mt5": 16388,  "bars": 2000,  "dd_limit": 50},
    "d": {"name": "D H1 Confluence",  "tf": "H1", "tf_mt5": 16385,  "bars": 3000,  "dd_limit": 50},
    "e": {"name": "E H1 Donchian",    "tf": "H1", "tf_mt5": 16385,  "bars": 3000,  "dd_limit": 50},
    "f": {"name": "F M15 Turbo",      "tf": "M15","tf_mt5": 15,     "bars": 10000, "dd_limit": 50},
    "g": {"name": "G M15 Confidence", "tf": "M15","tf_mt5": 15,     "bars": 10000, "dd_limit": 50},
}
        ...
        for fpath in sorted(glob.glob(os.path.join(ticker_dir, "strategy_*.json"))):
            ...
            if label in STRATEGY_MAP:
                ...
                if label == "a":
                    ...
                    params.setdefault("running_pct", 0)  # validasi: running_pct=0
...
def spinner_thread():
    ...
    while spinner_running:
        sys.stdout.write(f"\r  [LOADING] {chars[i % 4]}")
        sys.stdout.flush()
        time.sleep(0.1)
        i += 1
...
def main():
    sp = start_spinner()
    print("=" * 90)
    print("  VALIDASI 3 BLN - SPREAD REAL (running_pct=0, spread per trade)")
    ...
    for ticker in TICKERS:
        sym = ticker["name"]
        spread_cost_str = f"{ticker['spread']}pts x {ticker['point']} = {ticker['spread'] * ticker['point']}"
        stop_spinner(sp); sp = start_spinner()
        print(f"\n{'='*90}")
        print(f"  {sym} | spread {spread_cost_str} | modal Rp{ticker['modal']:,}")
        print(f"{'='*90}")
        ...
        for sc in strategies:
            ...
            target_ok = avg_daily >= ticker["target"]

            all_results.append({
                "ticker": sym,
                "strategy": name, "tf": tf,
                "days": days, "bars": len(df), "trades": len(trades),
            ...
            status = "OK" if target_ok else "NO"
            print(f"ROI {roi:+.1f}% DD {dd_max:.1f}% PF {pf:.2f} "
                  f"Rp{avg_daily:,.0f}/hr target Rp{ticker['target']:,} {status}"
                  f" | {len(trades)}t WR {len(win)/max(len(trades),1)*100:.0f}%")

    # SUMMARY
    stop_spinner(sp)
    print("\n\n")
    print("=" * 90)
    print("  RINGKASAN VALIDASI (spread real, running_pct=0)")
    print("=" * 90)

    header = (f"{'Ticker':<10} {'Strategi':<20} {'TF':<5} {'ROI':>8} {'DD':>6} "
              f"{'PF':>5} {'Rp/hr':>12} {'Target':>10} {'Status':>8} {'Trades':>6} {'WR':>5} {'ROI/bln':>8}")
    print(header)
    print("-" * 90)
    ...
    for r in sorted(all_results, key=lambda x: x["avg_daily"], reverse=True):
        ...
        roi_bln = r["roi"] / (r["days"] / 30) if r["days"] > 0 else 0
        print(f"{r['ticker']:<10} {r['strategy']:<20} {r['tf']:<5} "
              f"{r['roi']:>+7.1f}% {r['dd']:>5.1f}% {r['pf']:>4.2f} "
              f"{avg_str:>12} {target_str:>10} {status:>8} {r['trades']:>5d} {r['win_rate']:>4.0f}% {roi_bln:>+7.1f}%")
